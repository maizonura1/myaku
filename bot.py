...
def confirm_entry(df, direction):
    ...
    if direction == "LONG":
        ...
        # Breakout candle di prev (bukan current)
        breakout_candle_prev = prev['close'] > resistance and prev['close'] > prev['open'] * 1.003
        
        # Apakah current sedang retest (harga menyentuh area resistance)
        is_retesting = current['low'] <= resistance + (resistance * 0.0015)
        
        # Jangan entry jika candle SEDANG breakout
...
def live_open(sym, direction, score, sigs, price, atr_pct, sl_pct, tp_pct):
    ...
    # 2. Tukar persentase/jarak TP dan SL
    ...
    pos = {
        ...
        "tp_pct": inv_tp_pct,
        "partial_hit": False,
        "peak": price,          # Setup trailing stop valid untuk arah baru
        "trough": price         # Setup trailing stop valid untuk arah baru
    }
    ...
    print(f"\n  {d} [DRY INVERSE] {sym} {inv_direction} @{price:.6g} SL:{inv_sl_pct*100:.2f}% TP:{inv_tp_pct*100:.2f}% [{' | '.join(sigs[:5])}]")
