"""
Bot Scalping v19.2 — REVERSE LOGIC EDITION (LIVE)
====================================================
- PATCHED: Semua logic sinyal DIBALIK 100% (LONG = SHORT, SHORT = LONG).
- PATCHED: RR Ditukar (Target Profit lebih pendek dari Stop Loss).
- PATCHED: ImpatientLoss diubah menjadi ImpatientProfit (Bungkus cuan cepat).
"""
...
# RR DITUKAR POSISINYA
EXTREME_PROFIT_PCT = 0.0015  # +0.15% (Menggunakan angka HardSL yang lama agar gampang kesentuh)
HARD_SL_PCT        = 0.0020  # -0.20% (Menggunakan angka Profit yang lama agar lebih jarang kesentuh)
...
# STATS DIPERBARUI SESUAI NAMA LOGIKA BARU
_stats = {
    "trades": 0, "wins": 0, "losses": 0, "pnl": 0.0, "best": 0.0, "worst": 0.0,
    "hard_profit": 0, "extreme_loss": 0, "impatient_profit": 0, "force": 0,
...
def signal(df):
    ...
    # 🚨 PERUBAHAN LOGIKA DIBALIK (TUKAR POSISI RETURN LONG & SHORT) 🚨
    if lp <= sp or lp < thresh or gap < MIN_GAP:
        ...
        if INVERSE_MODE: return "SHORT", sp, ss[:3] + ["(REV-INV)"], atr
        return "LONG", sp, ss[:3] + ["(REVERSE)"], atr

    if br <= BR_LONG_MIN: return None, lp, [], atr
    if INVERSE_MODE: return "LONG", lp, sl[:3] + ["(REV-INV)"], atr
    return "SHORT", lp, sl[:3] + ["(REVERSE)"], atr
...
def live_close(sym, reason):
    ...
    # REKAP STATISTIK REASON BARU
    if "HardProfit" in reason: _stats["hard_profit"] += 1
    elif "ExtremeLoss" in reason: _stats["extreme_loss"] += 1
...
def monitor_positions():
    for sym in list(live_positions.keys()):
        ...
        # 🚨 PERUBAHAN NAMA & LOGIKA TAKE PROFIT / STOP LOSS 🚨
        if prof_pct >= EXTREME_PROFIT_PCT: live_close(sym, "HardProfit"); continue
        if prof_pct <= -HARD_SL_PCT: live_close(sym, "ExtremeLoss"); continue
        if hold >= MAX_HOLD_SEC: live_close(sym, "ForceTimeout"); continue
        
        # ImpatientProfit: Kalau sudah 15 detik harganya profit sedikit (+0.05%), langsung amankan!
...
def print_inline():
    ...
    print(f"     ┌ [v19.2 REVERSE] {n}T WR:{wr:.0f}% W:{_stats['wins']} L:{_stats['losses']} {e}PnL:{_stats['pnl']:+.4f}U")
    print(f"     └ Hard-Prof:{_stats['hard_profit']} Ex-Loss:{_stats['extreme_loss']} Imp-Prof:{_stats['impatient_profit']} Force:{_stats['force']}")
...
def print_full():
    ...
    e = "💚" if _stats["pnl"] >= 0 else "🔴"
    print(f"\n  {'─'*62}")
    print(f"  🚀 LIVE v19.2 [REVERSE LOGIC] — {sess*60:.0f}m | {tph:.1f}T/jam")
...
def run_bot():
    print("╔═══════════════════════════════════════════════════════╗")
    print(f"║  🚀 LIVE TRADE v19.2 — REVERSE LOGIC EDITION          ║")
    print("╚═══════════════════════════════════════════════════════╝\n")
